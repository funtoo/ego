...
# The source code of this file (and this file only) are distributed under an as-is
# license. See https://pypi.python.org/pypi/tabulate.
...
def _moin_row_with_attrs(celltag, cell_values, colwidths, colaligns, header=""):
	...
	values_with_attrs = [
		"{0}{1} {2} ".format(celltag, alignment.get(a, ""), header + c + header) for c, a in zip(cell_values, colaligns)
	]
...
def _latex_line_begin_tabular(colwidths, colaligns, booktabs=False):
	...
	return "\n".join(["\\begin{tabular}{" + tabular_columns_fmt + "}", "\\toprule" if booktabs else "\hline"])
